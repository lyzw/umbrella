...
6. 家长通过 `GET /family/children` 分页查询当前家庭的儿童申请/关系，再使用返回的 childId/applyId 同意和审批。儿童通过 `GET /family/binding` 查询自己的申请/绑定状态，无申请为 NONE；不暴露档案字段。
7. `GET /child/profile` 仅所属家长在 BOUND + 有效同意后回显；未建档404。偏好 GET 允许本人儿童/所属家长，PUT 仅儿童本人，只接收 dislikes/tastes，拒绝所有额外字段；要求已有 COMPLETE 档案，缺失/不完整409/E-002。

拒绝后重申复用原关系 ID，递增 `application_version`。同意记录关联 `apply_id + application_version`；旧申请同意不能授权新一轮绑定。数据库生成列唯一索引补强儿童单家庭约束。

## 事务与撤回

授权写入顺序固定为：家庭行 → 儿童账号行 → 成员关系行 → 最新同意记录。加入申请、同意、审批、档案、偏好与撤回使用同一顺序，档案/偏好的授权读取也采用锁定读。

儿童先用自己的 BOUND 关系定位家庭，取得范围锁后重读关系；不接受请求体中的目标身份。儿童同意人由该家庭 owner 推导并检查家长账号和 BOUND 关系，不能把儿童当作 GRANT 发起人。最新同意仍按家长、家庭、儿童、申请和申请版本定位，检查当前协议、有效期及核验策略。此实现限 Sprint 1 单创建家长模型，多监护人必须另行设计。

同意/撤回仅追加历史，重复撤回不新增业务事件；撤回成功返回200/REVOKED，不使家长会话失效。撤回提交后后续档案及偏好读写409/E-010；此前已经提交的合法数据保留历史，不能自动逆转。最小关系状态、同意查询及隐私请求仍可用，不返回已失去授权的档案内容。
...
收口新增审计动作 FAMILY_QUERY、BIND_QUERY、PROFILE_QUERY、PREFERENCES_QUERY、PREFERENCES；只保存作用域、同意 ID/版本及分页数值，不保存档案或偏好正文。偏好审计失败与业务更新一起回滚。本轮不改变八表结构，favoriteDishIds/收藏留在 Sprint 3，不用空数组冒充已完成收藏存储。

## 配置与发布

默认关闭 Q-01 数据处理；生产必须显式提供密钥和正式协议、目录及审批引用。dev/test 配置只允许合成演示，不代表生产签核。

`ProductionConfigGuard` 拒绝 prod 与 dev/test 混用、已知演示密钥和非30分钟 TTL；启用采集时还校验正式配置占位是否齐备。关闭闸门后，已有会话也不能选择角色、新建家庭、申请绑定或读写档案/偏好；撤回、最小关系/同意/权利状态查询和权利申请继续可用。

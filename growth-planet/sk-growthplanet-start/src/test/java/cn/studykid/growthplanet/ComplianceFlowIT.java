...
class ComplianceFlowIT extends BaseIT {
    ...
    void exportKeyCannotBeReusedForAnotherChild() throws Exception {
        ...
        String childToken = loginAndSelectRole(java.util.UUID.randomUUID().toString(),
                RoleEnum.CHILD);
        var join = mockMvc.perform(post("/api/mini/family/join").header("Authorization", "Bearer " + childToken)
        ...
        grant(second);
        approve(second);
        mockMvc.perform(post("/api/mini/compliance/data-export").header("Authorization", "Bearer " + first.parentToken())
    ...
    void invalidAgeAgreementVersionOrOwnershipCannotGrant() throws Exception {
        ...
        mockMvc.perform(post("/api/mini/compliance/consent").header("Authorization", "Bearer " + other.parentToken())
    ...
    void approvalRequiresConsentAndProfileRequiresBoundParent() throws Exception {
        ...
        grant(ctx);
        approve(ctx);
        mockMvc.perform(post("/api/mini/child/profile").header("Authorization", "Bearer " + ctx.childToken())
                .contentType(JSON).content(profileJson(ctx))).andExpect(status().isForbidden());
        mockMvc.perform(post("/api/mini/child/profile").header("Authorization", "Bearer " + ctx.parentToken())
    ...
    void exportIsDurableIdempotentAndPrivate() throws Exception {
        ...
        String taskId = null;
        for (int i = 0; i < 2; i++) {
            var result = mockMvc.perform(post("/api/mini/compliance/data-export")
        ...
        mockMvc.perform(get("/api/mini/compliance/requests/" + taskId)

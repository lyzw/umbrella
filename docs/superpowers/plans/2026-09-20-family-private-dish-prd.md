...
**待确认项 C-01（数据迁移方案）**：需选择迁移策略，编码前出变更文档：
- **方案 A（推荐）**：`dish_ids` 改为对象数组，历史数据一次性迁移（所有旧 ID 默认补 `{type:"PRESET", id:旧值}`），修改 `chk_menu_dishes` 约束为校验对象数组。优点：来源清晰、可追溯；代价：需迁移 + 改约束 + 改读写逻辑。
- **方案 B**：保留 `dish_ids` 纯 ID 数组（仅预置），新增 `dish_family_ids` 列存家庭菜品 ID 数组。优点：向后兼容、历史数据不动；代价：两列分别校验、合并展示，约束分散。

**待确认项 C-02（菜单快照）**：发布菜单时是否冻结菜品价格快照？
- 当前设计：不快照，菜品编辑即时影响已发布菜单（如改价后儿童点单按新价扣款）。
- 备选：发布时快照 `{type, id, name, price, image_url}`，编辑菜品不影响历史菜单。代价：菜单冗余、改价不追溯。**建议本期不快照**（与现有预置菜品行为一致）。

**待确认项 C-03（乐观锁）**：是否给 `life_menu_daily` 增加 `version` 列支持乐观锁？
- 现有表无 `version` 列。本期若要支持并发冲突提示（E-09），建议新增 `version INT NOT NULL DEFAULT 0`。涉及 DDL 变更，编码前出变更文档。
...
| 编号 | 待确认问题 | 影响范围 | 建议 |
|------|-----------|----------|------|
| C-01 | `dish_ids` 结构迁移方案 A/B | 数据模型、读写逻辑 | 方案 A（对象数组） |
| C-02 | 菜单是否冻结价格快照 | 儿童端计价一致性 | 不快照（与预置一致） |
| C-03 | `life_menu_daily` 是否新增 version 乐观锁列 | 并发冲突处理 | 新增 version 列 |
| C-04 | 可编排日期范围（默认未来 7 天？） | 编排入口约束 | 待用户确认 |
| C-05 | 同家庭菜名是否允许重复 | 数据唯一性 | 允许重复（个性化） |
| C-06 | 菜品图片上传走现有通道还是独立 OSS | 实现成本 | 沿用现有小程序上传通道 |
...
> **下一步**：请用户确认本 PRD，并对 C-01~C-06 待确认项给出决策。确认后进入「数据模型变更文档 + 接口设计」阶段，再编码。

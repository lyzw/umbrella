...
- 截至 2026-09-24，`node --test tests/*.test.js` 共 117 项通过，包含主题 token、角色串色和可访问性样式静态断言。
...
- [x] 增加并复用 primary-shadow、positive-ink、warning-ink、danger-ink 等语义 token，替换全局错误态、危险操作和状态徽标中的硬编码颜色；health 专用 token 仍待后续页面盘点。
- [x] 复核家长主题下底部导航、勋章激活态和钱包趋势图，避免儿童蓝色主色残留；通知、收藏和页面图标仍需逐页复核。
- [ ] 统一通知已读/未读、成功/警告/错误状态的颜色、图标和文案层级。
- [ ] 统一审批明细中的“单价、数量、小计、虚拟单位”字段表达。
- [x] 为返回、偏好删除和配方行内操作补齐 44px 最小触控高度；首页长标题和登录角色说明允许换行并提升说明字号。
- [ ] 在真机校验正文和小字对比度、禁用态辨识度、系统字体放大表现。
- [ ] 减少 emoji 和特殊字形承担关键操作语义；关键按钮使用稳定文字或项目既有图标方案。

**2026-09-24 本批完成项：**

- 角色主色、状态文字色和主按钮/勋章阴影已抽为语义 token；导航组件自行按 `parent` 角色覆盖 token。
- 钱包趋势图按 `PARENT`/`CHILD` 选择对应主色，坐标轴和网格继续使用中性色。
- 返回、偏好删除、配方行内操作最小高度统一到 44px；首页列表标题和登录角色说明支持长文本换行。
- 自动化已验证；微信开发者工具原生编译仍因缺少 `wcc` 未完成，系统字体放大、读屏和真实设备宽度仍待验收。
